...
//	private final HashMap<String, Object> cookies; // Request cookies
...
//		this.auth = Authorization.get(this);
		...
		this.formQueries = contentType.startsWith("application/x-www-form-urlencoded")
				? parseRawQuery(Utils.streamToString(body))
				: new HashMap<>();

		// Parse query and cookies, both returns not null if there is nothing
		this.queries = parseRawQuery(exchange.getRequestURI().getRawQuery());
//		this.cookies = RequestUtils.parseCookies(headers);
	...
	/**
	 * Pipe the body from this request to an file. If the file not exists, it will
	 * be created.
	 *
...
//	}
	 ...
	 */
	public void addMiddlewareContent(Middleware middleware, Object middlewareData) {
		this.middleware.put(middleware.getName(), middlewareData);
	}

	/**
	 * Get the data from a specific middleware by name (Also the reason why the
	 * interface ExpressFilter implements a getName())
	 *
	...
	/**
	 * Checks if the connection is 'fresh' It is true if the cache-control request
	 * header doesn't have a no-cache directive, the if-modified-since request
	 * header is specified and last-modified request header is equal to or earlier
	 * than the modified response header or if-none-match request header is *.
	 *
	 * @return True if the connection is fresh, false otherwise.
	 */
	public boolean isFresh() {

		if (headers.containsKey("cache-control") && headers.get("cache-control").get(0) != null
				&& headers.get("cache-control").get(0).equals("no-cache"))
			return true;

		if (headers.containsKey("if-none-match") && headers.get("if-none-match").get(0) != null
				&& headers.get("if-none-match").get(0).equals("*"))
			return true;

		if (headers.containsKey("if-modified-since") && headers.containsKey("last-modified")
	...
	public boolean isXHR() {
		return headers.containsKey("X-Requested-With") && !headers.get("X-Requested-With").isEmpty()
...
//	}
	...
	/**
	 * @return True if there was an Authorization header and the Authorization
...
//	}
	...
	/**
	 * Returns a query from a form which uses the
	 * 'application/x-www-form-urlencoded' request header.
	 *
	 * @param name The name.
	...
	public Integer intValue(String param) {
		return Integer.parseInt(getFormQuery(param));
	}
