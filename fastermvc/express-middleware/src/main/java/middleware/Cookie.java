...
/**
 * @author Simon Reinisch Simple class which represents an HTTPCookie
 */
public class Cookie {
	...
	/**
	 * Set the if the cookie shouldn't accessible via JavaScript through the
	 * Document.cookie property Default is false
	 *
	...
	public String toString() {
		...
		if (expire != null)
			b.append("; Expire=").append(expire);
		if (maxAge != -1)
			b.append("; Max-Age=").append(maxAge);

		if (domain != null)
			b.append("; Domain=").append(domain);
		if (sameSite != null)
			b.append("; SameSite=").append(sameSite);
